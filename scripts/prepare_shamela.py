...
ARCHIVE_DIST = (
	Path(__file__).resolve().parent / ".." / "datasets_archive" / "shamela-database-1448.zip"
)

shamela_extractor_relative = "tools/dataset-prep/shamela-extractor"
shamela_extractor = Path(
	os.path.join(__file__, "..", shamela_extractor_relative)
).resolve()

dataset_preparer_relative = "tools/dataset-prep/dataset-preparer"
shamela_extractor = Path(
	os.path.join(__file__, "..", dataset_preparer_relative)
).resolve()
